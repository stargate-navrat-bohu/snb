...
/*  if($id=="$banf[ip]") {
    ...
    if($cas >= $banf["konec"]){
      ...
      $konec1 = $banf["konec"];
      echo "<font color='cc4030' size='5px' family='Times New Roman' Weight='bold'>Protože: ".$banf["duvod"].". Byl vám pøístup na fora zakázán. Ban je udìlen do ".Date("G:i:s j.m.Y",$konec1).".</font>";
      die;
    }
  }
}
...
if($kde==""){
  ...
  echo "<font class='tabulka'>";
    ...
    echo "<a href='hlavni.php?page=seznamodmen'>Seznam odmìn</a>&nbsp;|&nbsp;";	
...
//------------------------------------------------------------------------------
      echo"<form method='post' name='enter' action=hlavni.php?page=forum&kde=".$kde.">";
        ...
        <tr>
          <td class="nadpis" valign="top"><b>Text:</b> </td>
          <td><textarea name='text' cols='75' rows='6' class="textarea"><?php echo $text; ?></textarea></td>
        </tr>
        <tr>
          <td colspan=2 align="center">
    <?
      ...
      ?><br><br>
      <input type='hidden' value='<? echo $kde;?>' name='kde'>
      <input type='hidden' value='<?php echo $_POST["edit"]; ?>' name='edit_set' >
