...
unset($planety_obchod);
  ...
  //$logcislo=$zaznam1[logcislo];
  //$vys8 = MySQL_Query("SELECT * FROM obchodlog where cislo=$logcislo");
...
<?	
$diletace=72;
if((Date("U")-$zaznam1[vek])<($diletace*3600)){echo "<font class='text_cerveny'>Nemùžete obchodovat. Nejste tu ještì ani $diletace hodin a tak nejste zapsán v obchodním rejstøíku.</br>Obchod pokazeny</font>";exit;}
...
if($casted-$obchodod<=300 and $co!=2 and $co!=3 and $co!=4 and $co!=5){echo "<center><font class='text_cerveny'>Další obchod je možný až za ".number_format($b,1,"."," ")." minut</font></center>";die;};
    ...
    $j1=abs($j1);
      ...
      //if(!(is_int($cj1) and is_int($cj2) and is_int($cj3) and is_int($cj4))){echo "<font class='text_cerveny'>Zadána mohou být jen èísla</font>";break;};
			...
			//if(!(is_int($j1) and is_int($j2) and is_int($j3) and is_int($j4))){echo "<font class='text_cerveny'>Zadána mohou být jen celá èísla</font>";break;};
		
			//if($j1!=NULL)
     /* if(isset($j1))
      { 
      $stringj1=$j1;
      settype($stringj1, "string");
      if(!ctype_digit($stringj1)){echo "<font class='text_cerveny'>Zadána mohou být jen èísla</font>";break;}
      }*/
      //if($j2!=NULL){if(!ctype_digit($j2)){echo "<font class='text_cerveny'>Zadána mohou být jen èísla</font>";break;};}
			//if($j3!=NULL){if(!ctype_digit($j3)){echo "<font class='text_cerveny'>Zadána mohou být jen èísla</font>";break;};}
			//if($j4!=NULL){if(!ctype_digit($j4)){echo "<font class='text_cerveny'>Zadána mohou být jen èísla</font>";break;};}
	...
	while($obchod = MySQL_Fetch_Array($obch)):
			...
			$datum = Date("G:i j.m.Y",$den);
				...
				$cj4=$obchod[cj4];
				  ...
				  //if(Is_double($pj1) or Is_double($pj2) or Is_double($j3) or Is_double($pj4)){echo "<font class='text_cerveny'>Èísla nesmí být desetinné</font>";break;};	
    	    //if($pj1<0 or $pj2<0 or $j3<0 or $pj4<0){echo "<font class='text_cerveny'>Èísla musí být vìtší jak nula</font>";break;};
          //if(!(ctype_digit($pj1) and ctype_digit($pj2) and ctype_digit($j3) and ctype_digit($pj4))){echo "<font class='text_cerveny'>Zadána mohou být jen èísla</font>";break;};	
...
//echo "<font class='text_cerveny' size='18'>Obchod s planetami uzavøen!!! Všchy vložené planety byly nenávratnì a bez náhrady straceny!!! Pøíštì už možná bugovat nebudete!!!</font>";exit;
