...
.zmraz{color:#5555ee; font-family:Times New Roman; text-align:center;}
.zmrazy{color:#ffffff; font-family:Times New Roman; text-align:center; background:#cc6600;}
.ty{color:#ffffff; font-family:Arial; text-align:center; background:#474747; }
</style>
...
?>
	<a href="hlavni.php?page=vesmiramp;&jak=smlouvy&amp;nova=1">Nová smlouva</a><br><br>
<?php
	if($nova==1){
?>
		<form action="hlavni.php?page=vesmir&amp;jak=smlouvy" method="post">
		<table width="95%">
		<tr><td>s rasou</td><td>možné vztahy</td><td>výpovědní lhůta(v hodinách)</td><td>poznámka(možno použít html tagy)</td></tr>
		<tr>
		<td>
		<select name="rasa2">
		<?php
...
?>
		...
		Neútočení <input type="checkbox" name="neutoceni" value="1"><br>
...
<table width="95%"  border="1" cellpadding="5px">
  <tr>
    ...
    <td><strong>výpovědní lhůta </strong></td>
    ...
    for($i=0;$i<5;$i++) {
        if($array_vztahy_decoded[$i] != 0) {
            switch($i) {
                ...
                case 1: echo "<font color=#00dddd>neútočení </font><br>";break;
...
function status_decode($status_data,$vyprsi_data,$id) {
    ...
    switch($status_data) {
        case 0: echo "<font color=yellow>Čeká na podepsání</font>";break;
        case 1: echo "<font color=green>Podepsaná</font>";break;
        case 2: echo "<font color=yellow>Běží výpovědní lhůta<br>Vypr��:<br>".date("d. m. Y",$vyprsi_data) ."<br>". date("H:i:s",$vyprsi_data)."</font>";break;
        case 3: echo "<font color=red>Vypovězená smlouva - ".date("d. m. Y",$vyprsi_data) ."<br>". date("H:i:s",$vyprsi_data)."</font>";break;
    }
}
...
?>
                <a href="hlavni.php?page=vesmir&amp;jak=smlouvy&amp;vypovedet=1&amp;rasa=<?php echo $rasa;?>&amp;id=<?php echo $array_smlouvy['id']; ?>">Vypovědět</a>
<?php
            }
        }
    }
...
?>
    <tr>
        ...
        check_vudce($array_vlada,$logjmeno,$array_smlouvy);
