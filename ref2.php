...
if(isset($zmez)) {
  do {
    if($zaznam1[admin]!=1)
    if ($zaznam1[status]!=1) {
	  echo "<h1>Toto je dovoleno pouze admin�m a v�dc�m.</h1>";
	  break;
	};	
	...
	$vysledek="Na otazku: \'".$ref[otazka]."\' pro odpov��: ".$ref[odpoved1]." hlasovalo ".$ref[odp1].", pro odpov��: ".$ref[odpoved2]." hlasovalo ".$ref[odp2]." a pro odpov��: ".$ref[odpoved3]." hlasovalo ".$ref[odp3].". Celkem hlasovalo lid�: ".$dohromady . " / " . $hhh;
	if ($hhh!=0) {$vysledek.= " (". Round(($ref[odp1]+$ref[odp2]+$ref[odp3])/$hhh*100) ."%).";} else {$vysledek.=".";};
...
// Hlasovani v narodnim referendu - konec
	...
	echo "<h6><font class=kapital>C</font>elovesm�rn� referendum.</h6><font class='info'>";
	echo "Ot�zka zn�: <font class=pole>".$ref[otazka]."</font><br>";
	echo "Odpov�� 1: <font class=pole>".$ref[odpoved1]."</font><br>";
	echo "Odpov�� 2: <font class=pole>".$ref[odpoved2]."</font><br>";
	echo "Odpov�� 3: <font class=pole>".$ref[odpoved3]."</font><br>";
	echo "Zat�m odpov�d�lo: <font class=pole>";
	echo $ref[odp1]+$ref[odp2]+$ref[odp3] . " / " . $hhh;
	if ($hhh!=0) {echo " (". Round(($ref[odp1]+$ref[odp2]+$ref[odp3])/$hhh*100) ."%)</font>";};
	echo "<br>Odpov�� 1 zvolilo: <font class=pole>".$ref[odp1];
	if (($ref[odp1]+$ref[odp2]+$ref[odp3])!=0) echo " (". Round($ref[odp1]/($ref[odp1]+$ref[odp2]+$ref[odp3])*100) ."%)";
	echo "</font>. Odpov�� 2 zvolilo: <font class=pole>".$ref[odp2];
	if (($ref[odp1]+$ref[odp2]+$ref[odp3])!=0) echo " (". Round($ref[odp2]/($ref[odp1]+$ref[odp2]+$ref[odp3])*100) ."%)";
	echo "</font>. Odpov�� 3 zvolilo: <font class=pole>".$ref[odp3];
	if (($ref[odp1]+$ref[odp2]+$ref[odp3])!=0) echo " (". Round($ref[odp3]/($ref[odp1]+$ref[odp2]+$ref[odp3])*100) ."%)";
	echo "</font>";
	echo "<form method='post' action='ref2.php'>";
	echo "<input type='radio' name='r' value='odp1' ".$odp1."> Odpov�� 1 &nbsp;&nbsp;&nbsp;&nbsp;
  <input type='radio' name='r' value='odp2' ".$odp2."> Odpov�� 2 &nbsp;&nbsp;&nbsp;&nbsp;
  <input type='radio' name='r' value='odp3' ".$odp3."> Odpov�� 3 ";
	echo "<br><br><input type='submit' value='zm�� volbu'>";
	echo "</form>";
	echo "V�sledek minul�ho referenda: <font class=pole>".stripslashes($ref[vysledek])."</font></font>";
	...
	$vys5 = MySQL_Query("SELECT jmeno FROM uzivatele where rasa='$rasa'");
	$hhh = mysql_num_rows($vys5);
	echo "<h6><font class=kapital>N</font>�rodn� referendum.</h6><font class='info'>";
	echo "Ot�zka zn�: <font class=pole>".$ref[otazka]."</font><br>";
	echo "Odpov�� 1: <font class=pole>".$ref[odpoved1]."</font><br>";
	echo "Odpov�� 2: <font class=pole>".$ref[odpoved2]."</font><br>";
	echo "Odpov�� 3: <font class=pole>".$ref[odpoved3]."</font><br>";
  ...
  <input type='radio' name='r' value='odp3' ".$odp3."> Odpov�� 3 ";
	...
	if($zaznam1[status]==1 || $zaznam1[status]==5 || $zaznam1[status]==2):
		echo "<form method='post' action='ref2.php'>";
		echo "<h6><font class=kapital>Z</font>m�nit narodn� referndum.</h6><font class='info'>";
		echo "Ot�zka: <input type=text name=znar size=80><br>";
		echo "Odpov�� 1: <input type=text name=nodpoved1 size=80><br>";
		echo "Odpov�� 2: <input type=text name=nodpoved2 size=80><br>";
		echo "Odpov�� 3: <input type=text name=nodpoved3 size=80><br>";
	...
	if($zaznam1[admin]==1):
		echo "<form method='post' action='ref2.php'>";
		echo "<h6><font class=kapital>Z</font>m�nit celovesm�rn� referendum.</h6><font class='info'>";
		echo "Ot�zka: <input type=text name=zmez size=80><br>";
		echo "Odpov�� 1: <input type=text name=codpoved1 size=80><br>";
		echo "Odpov�� 2: <input type=text name=codpoved2 size=80><br>";
		echo "Odpov�� 3: <input type=text name=codpoved3 size=80><br>";
		echo "<br></font><input type='submit' value='zm�� referendum'>";
		echo "</form>";
	endif;
