...
mysql_query("SET NAMES cp1250");
	...
	$akce2="Proveï akce u zaškrtnutých záznamù";
		...
		while($am = MySQL_Fetch_Array($am2)):
			...
			if($$co=='on')://echo "_$am[jmeno]_<br>";
				...
				if($kolik<4):
					...
					switch($kolik){
						case 1: $status="první varování";
							break;
						case 2: $status="druhé varování";
							break;
						case 3: $status="tøetí a poslední varování";
							break;
					}		
					$co="Varujeme vás. Existuje podezøení, že máte více než jeden login. To je porušení pravidel, za které vám hrozí smazání. V pøípadì, že máte skuteènì více loginù, tak je smažte. V pøípadì opaèném v sekci Osobní nastavení tento stav odùvodnìte. Toto je ".$status.".";
            ...
            MySQL_Query("update uzivatele set posta = posta + 1 where jmeno = '$adresat'");
					...
					if($proved==0):
						...
						echo "<h1>Pošta se bohužel nepodaøila poslat, zkuste pozdìji.</h1>";
					endif;
				else:
					$vys9 = MySQL_Query("SELECT * FROM uzivatele where cislo = $cislo");
					$zaznam9 = MySQL_Fetch_Array($vys9);
          ...
          $text.= "byl smazán";
					...
					$zprava="Toto je informaèní zpráva ze hry SG-RTG (www.sg-rtg.net). Jelikož jste porušil(a) pravidla hry, tak byl váš profil ".$hracn." smazán.";
					mail ($email,"Informaèní zpráva z SG-RTG",$zprava);
	...
	if(isset($pridat)):
		...
		if($amm[cislo]!=$cislo):
			...
			$kolik=1;
			$co="Varuji Vás, porušejete pravidla, máte pøíliš loginù. Buïto je smažte nebo napište v osbních nastaveních dùvod. Toto je první varování";
			$nazev = "Muláctví";
      $text=HTMLSpecialChars($co);
			$text=NL2BR($text);
			$text=AddSlashes($text);
...
<h6><font class=text_modry>P</font>øidání nìkoho do tabulky</h6> 
...
<a href="javascript:void(0);" onclick="checkAll(document.getElementById('myform'), 'filecheck', false); checkAll(document.getElementById('myform'), 'filecheck2', true);">vybrat neodùvodnìné</a>&nbsp;
...
<a href='?podle=1' id=qba onMouseOver = Rozsvitit('qba') onMouseOut=Zhasnout('qba')>nejdøív odùvodnìní</a></h6>

<?echo "<TABLE ".$table." ".$border." width=100%>";?>
<tr>
<th>Jméno</th>
<th>posl. zmìna</th>
<th>odùvodnìní</th>
...
while($am = MySQL_Fetch_Array($am2)):
	...
	if ($am[varovani]==9) {
		echo "<td colspan='3'>";
		    ...
		    echo "<td>";}
	if ($am[varovani]!=9) {
		echo "<a href='hlavni.php?page=admin9&ocistit=$am[cislo]'>oèistit</a>";
	} else {
...
echo "</form></table>";
?>
<h6>Pøidání do antimulti teamu</h6> 
<form name='form' method='post' action='hlavni.php?page=admin9'>
<input type=text name=pan><input type=submit value="Pøidej">
</form>
...
while($at = MySQL_Fetch_Array($at2)):
	...
	echo "<td><input type=submit value='vyhoï'>";
