...
if(isset($hracn) and $_POST['over_post']==1):
		...
		do{
			...
			if($dobre!=1){echo "<h1>Neexistuj�c� login</h1>";break;};
			...
			echo "<h1>Hr��i $hracn posl�no $naq naq.</h1>";
			...
			$text="Hr��i $hracn posl�no jako kompenzace $naq naq.";
      ...
      MySQL_Query("INSERT INTO log (cil,den,cilt,akce,konk,admin) VALUES ('1',$den,'$hracn','posl�n naq hr��i',$naq,'$zaznam1[jmeno]')");
	...
	if(isset($rase) and $_POST['over_post']==1):
		...
		echo "<h1>Rase $zaznam3[nazevrasy] posl�no $naq naq.</h1>";
		...
		$text="Rase $zaznam3[nazevrasy] posl�no jako kompenzace $naq naq.";
	...
	//	MySQL_Query("INSERT INTO forum (den,jmeno,rasa,text,kde,stat) VALUES ('$den','$jmeno','$rasa','$text','$kde',$stat)");	
  	...
  	MySQL_Query("INSERT INTO log (cil,den,cilt,akce,konk,admin) VALUES ('3',$den,'$zaznam3[nazevrasy]','posl�n naq rase',$naq,'$zaznam1[jmeno]')");							
	...
	if(isset($hrach) and $_POST['over_post']==1):
    	...
    	echo "<h1>Hr��i $hrach heslo zm�n�no.</h1>";
	...
	if(isset($cp) and $_POST['over_post']==1):
		...
		do{
			...
			MySQL_Query("insert into log (cil,admin,akce,konk,cilt,den) values ('9','$zaznam1[jmeno]','vytvo�en� CP','$cp','$nazavrasyprocp',$den)");
	...
	if(isset($dbp) and $_POST['over_post']==1):
		...
		do{
			...
			MySQL_Query("insert into log (cil,admin,akce,konk,cilt,den) values ('10','$zaznam1[jmeno]','vytvo�en� PP','$dbp','$nazavrasyprodbp',$den)");
	...
	if(isset($posta) and $_POST['over_post']==1):
		...
		switch($posta){
			...
			case 0:
				...
				$text="Admin po�ta pro v�echny:<br>".$text;
			...
			case 1:
				...
				$text="Admin po�ta, pro v�echny v�dce:<br>".$text;
        ...
        case 3:
				...
				$text="Admin po�ta, pro v�echny u�itele:<br>".$text;
        ...
        $id=$id_posta_1[id]+1;
		    ...
		    MySQL_Query("update pocitani_id set id='$id' WHERE sekce='posta' ") or die(mysql_error());
				...
				echo "odesila se posta pro u�itele $adresat :)<br />";
	...
	if(isset($vyzkumco) and $_POST['over_post']==1):
		...
		do{
			...
			if($vyzkumkolik<0){echo "��sla mus� b�t v�t�� jak nula";break;};
	...
	if(isset($hjmeno) and $_POST['over_post']==1):
	   ...
	   $zaznam22 = MySQL_Fetch_Array($vys22);	
		...
		MySQL_Query("insert into log (cil,admin,akce,konk,cilt,den) values ('8','$zaznam1[jmeno]','vytvo�en� NH','$zaznam21[nazev]','$zaznam22[nazevrasy]',$den)");
...
Poslat �astku <input type='text' name="naq" size=10> naq hr��i <input type='text' name='hracn'><input type='hidden' name='over_post' value=1><input type='submit' value="po�li">
...
<input type='hidden' name='over_post' value=1><input type='submit' value='po�li'>	
...
Zm�nit hr��i <input type='text' name="hrach"> heslo na <input type='password' name="heslo" size=15><input type='hidden' name='over_post' value=1><input type='submit' value="zm��">
...
Vytvo� CP se jm�nem <input type='text' name="cp" size=20> pat��c� rase
...
<input type='hidden' name='over_post' value=1><input type='submit' value='vytvo�'>	
...
Vytvo� dob�vac� planetu se jm�nem <input type='text' name="dbp" size=20> pat��c� rase
...
<select name='posta'>
	...
	<?php //<option value=99>v�em ?>

	<option value=1>v�dc�m

	<?php //<option value=2>jednotlivci	?>

	<option value=3>u�itel�m

</select>

 P�edm�t: <input type='text' name='predmet' size='20'>	

<br><textarea name='text' rows=5 cols=60></textarea>

<br><input type='hidden' name='over_post' value=1><input type='submit' value='po�li'></form>



<form name='form6' method='post' action='hlavni.php?page=admin4'>

Vytvo� hrdinu <input type='text' name="hjmeno" size=20 value=jmeno> rase
...
<br><input type='hidden' name='over_post' value=1><input type='submit' value='vytvo�'></form>
...
/*
  ...
  if(isset($nuluj)):
    ...
    $border= 'border=1';
		...
		echo "<br><br><br><b><u>Nastaven� v�zkumu:</b></u>";
		...
		echo "<th>C�l v�zkumu</th>";

		echo "<th>Cena v�zkumu</th>";
      ...
      $rasa2[$row["rasa"]] = $row["nazevrasy"];
		...
		echo "<br></br><form>N�zev v�zkumu: <input type='text' size = 30 value='$zaznam44[vyzkum]' name='nazvyz'>";
		...
		echo "<input type='submit' value='vynuluj v�zkum'>";
...
$zaznam54 = MySQL_Fetch_Array($vys54);
		...
		echo "<br></br><form>Nastaven� ceny agenta (so�asn� cena je $zaznam54[jed7_cena] * v�roba jedn� t��rny) : <input type='text' size = 5 value='$zaznam54[jed7_cena]' name='agent'>";
...
/*		echo "<br></br><form>Cena zlod�je (so�asn� cena je $zaznam54[jed6_cena] * v�roba jedn� t��rny) : <input type='text' size = 5 value='$zaznam54[jed6_cena]' name='zlodej'>";
...
*/		echo "</form>";
  	...
  	echo "Postav� na ka�d� CP 28 000 v�roben ";

		echo "<input type='hidden' name='postavvyrovnynacp' value='1'>";

		echo "<input type='submit' value='postav v�robny na CP'>";
		...
		echo "Zpr�va dne(zobrazovana na infu): ";
