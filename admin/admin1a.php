...
<?		
     ...
     // MySQL_Query("INSERT INTO forum (den,jmeno,rasa,text,kde,stat,titulek,pohlavi,stat2,rasac,hrac_id) VALUES ('$den','$jmeno','$rasa','$text','$kde','$stat','','','','')");
      ...
      $v1 = MySQL_Query("SELECT jmeno,cislo,koho,email FROM uzivatele where cislo = '$smaz'");	
			...
			$zprava="Toto je informaèní zpráva ze hry stargate (http://sg-rtg.net). Jelikož jste porušil pravidla hry, tak byl váš profil $hracn smazán.";
...
<input type='submit' value='Zmìnit' name=odebrat></form></td>";
			echo "</tr>";
...
if(isset($hracn)) {
	...
	if(isset($posta)){
      if ($posta==""){echo "<h1>Musíte zadat jméno hráèe!</h1>";exit;}
    	if(empty($xm) or $xm<0){$xm=0;};
			$pla2 = mysql_Query("SELECT *
FROM posta
WHERE (
odesilatel = 'ETNyx'
OR adresat = 'ETNyx'
)
ORDER BY odeslano_kdy DESC
LIMIT 0 , 10");
	...
	echo "<a href=hlavni.php?page=admin1&xm=".$y."&cislo=".$cislo."&posta=".$posta." id=qq onMouseOver = Rozsvitit('qq') onMouseOut=Zhasnout('qq')>Starších 10 zpráv</a></h6>";
      ...
      while($planeta2 = mysql_Fetch_Array($pla2)):
			echo "<tr>";
			$p=Date("d.m.Y H:i:s",$planeta2[den]);
			echo "<td><center>$p</td>";
